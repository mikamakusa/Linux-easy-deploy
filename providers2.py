...
class Provider(object):
    ...
    class Server(object):
        ...
        def OVH(cls, ApplicationKey, SecretKey, EndPoint, Region, Image, Flavor, ServerName, ServerId, action):
            ...
            sig = "$1$" + s1.hexdigest()
            queryHeaders = {"X-Ovh-Application": ApplicationKey, "X-Ovh-Timestamp": time, "X-Ovh-Consumer": ConsumerKey, "X-Ovh-Signature": sig, "Content-type": "application/json"}
            service = requests.post("https://eu.api.ovh.com/1.0/cloud/project",headers=queryHeaders)
            # Get ImageId
            s1 = hashlib.sha1()
            s1.update("+".join([ApplicationKey, ConsumerKey, "GET", "https://eu.api.ovh.com/1.0/cloud/project/%s/image"%service,time]))
            sig = "$1$" + s1.hexdigest()
            queryHeaders = {"X-Ovh-Application": ApplicationKey, "X-Ovh-Timestamp": time, "X-Ovh-Consumer": ConsumerKey, "X-Ovh-Signature": sig, "Content-type": "application/json"}
            ...
            # Get FlavorId
            s1 = hashlib.sha1()
            s1.update("+".join([ApplicationKey, ConsumerKey, "GET", "https://eu.api.ovh.com/1.0/cloud/project/%s/flavor"%service,time]))
            sig = "$1$" + s1.hexdigest()
            queryHeaders = {"X-Ovh-Application": ApplicationKey, "X-Ovh-Timestamp": time, "X-Ovh-Consumer": ConsumerKey, "X-Ovh-Signature": sig, "Content-type": "application/json"}
            ...
            if action.get("Insert"):
                s1 = hashlib.sha1()
                s1.update("+".join([ApplicationKey, ConsumerKey, "GET", "https://eu.api.ovh.com/1.0/cloud/project/%s/instance"%service,time]))
                sig = "$1$" + s1.hexdigest()
                queryHeaders = {"X-Ovh-Application": ApplicationKey, "X-Ovh-Timestamp": time, "X-Ovh-Consumer": ConsumerKey, "X-Ovh-Signature": sig, "Content-type": "application/json"}
                _body = '{"flavorId": %s,"imageId": "%s","monthlyBilling": false,"name": "%s","region": "%s"}'% (FlavorId, ImageId, ServerName, Region)
                requests.post("https://eu.api.ovh.com/1.0/cloud/project/%s/instance"%service,headers=queryHeaders,body=_body)
            elif action.get("Remove"):
                s1 = hashlib.sha1()
                s1.update("+".join([ApplicationKey, ConsumerKey, "GET", "https://eu.api.ovh.com/1.0/cloud/project/%s/instance"%service,time]))
                sig = "$1$" + s1.hexdigest()
                queryHeaders = {"X-Ovh-Application": ApplicationKey, "X-Ovh-Timestamp": time, "X-Ovh-Consumer": ConsumerKey, "X-Ovh-Signature": sig, "Content-type": "application/json"}
                requests.delete("https://eu.api.ovh.com/1.0/cloud/project/%s/instance/%s"%(service, ServerId),headers=queryHeaders)
            elif action.get("Reboot"):
                s1 = hashlib.sha1()
                s1.update("+".join([ApplicationKey, ConsumerKey, "GET", "https://eu.api.ovh.com/1.0/cloud/project/%s/instance"%service,time]))
                sig = "$1$" + s1.hexdigest()
                queryHeaders = {"X-Ovh-Application": ApplicationKey, "X-Ovh-Timestamp": time, "X-Ovh-Consumer": ConsumerKey, "X-Ovh-Signature": sig, "Content-type": "application/json"}
                requests.post("https://eu.api.ovh.com/1.0/cloud/project/%s/instance/%s/reboot"%(service, ServerId),headers=queryHeaders)
            elif action.get("Rebuild"):
                s1 = hashlib.sha1()
                s1.update("+".join([ApplicationKey, ConsumerKey, "GET", "https://eu.api.ovh.com/1.0/cloud/project/%s/instance"%service,time]))
                sig = "$1$" + s1.hexdigest()
                queryHeaders = {"X-Ovh-Application": ApplicationKey, "X-Ovh-Timestamp": time, "X-Ovh-Consumer": ConsumerKey, "X-Ovh-Signature": sig, "Content-type": "application/json"}
                requests.post("https://eu.api.ovh.com/1.0/cloud/project/%s/instance/%s/reinstall"%(service, ServerId),headers=queryHeaders)
